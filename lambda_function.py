...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Create Transaction
        url2 = "https://"+datacenter+".qualtrics.com/API/v3/directories/"+directoryid+"/transactions"
        ...
        tx_id = json.loads(response2.text)['result']['createdTransactions']['usrTx']['id']
        print("transaction id:",tx_id)
        # Create Transaction Batch
        url3 = "https://"+datacenter+".qualtrics.com/API/v3/directories/"+directoryid+"/transactionbatches"
        payload3 = {
        ...
        payload4 = {
            "surveyId": survey_id,
            "linkType": "Individual",
            "description": "distribution "+datetime.now().strftime('%Y-%m-%d %I:%M:%S'),#"2019-06-24 00:00:00",
            "action": "CreateTransactionBatchdistribution",
            "transactionBatchId": batch_id,
            "expirationDate": (datetime.now() + relativedelta(months=+2)).strftime('%Y-%m-%d %I:%M:%S'),#"2019-07-24 00:00:00",
            "mailingListId": mlid
        }
        response4 = requests.request("POST", url4, json=payload4, headers=headers)
        print(response4.text)
        dist_id = json.loads(response4.text)['result']['id']
        print("distribution ID:",dist_id)
        # Fetch Distribution links
        url5 = "https://"+datacenter+".qualtrics.com/API/v3/distributions/"+dist_id+"/links?surveyId="+survey_id
